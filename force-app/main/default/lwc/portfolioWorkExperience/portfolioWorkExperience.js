...
export default class PortfolioWorkExperience extends LightningElement {
    ...
    formatExperience(data){
        this.workExperienceList = [...data.records].reverse().map(item=>{
            let id = item.id
            const {JobStartDate__c, JobEndDate__c, Role__c, WorkLocation__c, 
                CompanyName__c, Description__c, IsCurrent__c} = item.fields
                ...
                return {id,JobStartDate,JobEndDate,Role,WorkLocation,CompanyName,Description,IsCurrent}
